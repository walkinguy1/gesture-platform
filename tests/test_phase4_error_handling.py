...
class TestSignLanguageRegistry:
    ...
    def test_duplicate_language_error(self):
        ...
        registry = SignLanguageRegistry()
        metadata = SignLanguageMetadata(code='ASL', name='ASL')
    ...
    def test_register_language_with_force(self):
        """Test overwriting language with force=True."""
        registry = SignLanguageRegistry()
        metadata = SignLanguageMetadata(code='ASL', name='Modified ASL')

        # Should not raise with force=True
        registry.register_language(metadata, ['X', 'Y', 'Z'], force=True)
        assert registry.validate_symbol('X', 'ASL') is True
        assert registry.validate_symbol('A', 'ASL') is False  # Old symbols gone
    ...
    def test_set_active_language(self):
        """Test setting active language."""
        registry = SignLanguageRegistry()
        metadata = SignLanguageMetadata(code='JSL', name='Japanese Sign Language')
        registry.register_language(metadata, ['A', 'B'])

        registry.set_active_language('JSL')
        assert registry.get_active_language() == 'JSL'
    ...
    def test_empty_symbols_error(self):
        ...
        registry = SignLanguageRegistry()
        metadata = SignLanguageMetadata(code='TEST', name='Test')
...
class TestASLRecognizerErrorHandling:
    ...
    def test_invalid_feature_shape(self):
        ...
        recognizer = ASLRecognizer()
        recognizer.model = lambda: None  # Fake model to pass is_loaded
...
class TestModelLoaderErrorHandling:
    ...
    def test_save_invalid_model(self):
        """Test saving with None model."""
        with pytest.raises(ModelLoadError):
            ModelLoader.save(None, ['A', 'B'], 'model.pkl')

    def test_save_invalid_model_or_error(self):
        """Test saving with None model raises error."""
        with pytest.raises((ModelLoadError, ModelSaveError)):
            ModelLoader.save(None, ['A', 'B'], 'model.pkl')

    def test_save_invalid_classes(self):
...
class TestDynamicLanguageSupport:
    ...
    def test_track_predictions_per_language(self):
        """Test tracking predictions separately per language."""
        registry = SignLanguageRegistry()

        # Add a second language
        metadata = SignLanguageMetadata(code='CSL', name='Chinese Sign Language')
